 ...
 */
package ir.besteveryeverapp.telegram.exoplayer.extractor;
...
import ir.besteveryeverapp.telegram.exoplayer.C;
import ir.besteveryeverapp.telegram.exoplayer.MediaFormat;
import ir.besteveryeverapp.telegram.exoplayer.MediaFormatHolder;
import ir.besteveryeverapp.telegram.exoplayer.ParserException;
import ir.besteveryeverapp.telegram.exoplayer.SampleHolder;
import ir.besteveryeverapp.telegram.exoplayer.SampleSource;
import ir.besteveryeverapp.telegram.exoplayer.SampleSource.SampleSourceReader;
import ir.besteveryeverapp.telegram.exoplayer.TrackRenderer;
import ir.besteveryeverapp.telegram.exoplayer.drm.DrmInitData;
import ir.besteveryeverapp.telegram.exoplayer.extractor.flv.FlvExtractor;
import ir.besteveryeverapp.telegram.exoplayer.extractor.mp3.Mp3Extractor;
import ir.besteveryeverapp.telegram.exoplayer.extractor.mp4.FragmentedMp4Extractor;
import ir.besteveryeverapp.telegram.exoplayer.extractor.mp4.Mp4Extractor;
import ir.besteveryeverapp.telegram.exoplayer.extractor.ogg.OggExtractor;
import ir.besteveryeverapp.telegram.exoplayer.extractor.ts.AdtsExtractor;
import ir.besteveryeverapp.telegram.exoplayer.extractor.ts.PsExtractor;
import ir.besteveryeverapp.telegram.exoplayer.extractor.ts.TsExtractor;
import ir.besteveryeverapp.telegram.exoplayer.extractor.wav.WavExtractor;
import ir.besteveryeverapp.telegram.exoplayer.extractor.webm.WebmExtractor;
import ir.besteveryeverapp.telegram.exoplayer.upstream.Allocator;
import ir.besteveryeverapp.telegram.exoplayer.upstream.DataSource;
import ir.besteveryeverapp.telegram.exoplayer.upstream.DataSpec;
import ir.besteveryeverapp.telegram.exoplayer.upstream.Loader;
import ir.besteveryeverapp.telegram.exoplayer.upstream.Loader.Loadable;
import ir.besteveryeverapp.telegram.exoplayer.util.Assertions;
import ir.besteveryeverapp.telegram.exoplayer.util.Util;
  ...
  static {
    ...
    try {
      DEFAULT_EXTRACTOR_CLASSES.add(
          Class.forName("ir.besteveryeverapp.telegram.exoplayer.extractor.webm.WebmExtractor")
    ...
    try {
      DEFAULT_EXTRACTOR_CLASSES.add(
          Class.forName("ir.besteveryeverapp.telegram.exoplayer.extractor.mp4.FragmentedMp4Extractor")
              .asSubclass(Extractor.class));
    } catch (ClassNotFoundException e) {
      // Extractor not found.
    }
    try {
      DEFAULT_EXTRACTOR_CLASSES.add(
          Class.forName("ir.besteveryeverapp.telegram.exoplayer.extractor.mp4.Mp4Extractor")
              .asSubclass(Extractor.class));
    } catch (ClassNotFoundException e) {
      // Extractor not found.
    }
    try {
      DEFAULT_EXTRACTOR_CLASSES.add(
          Class.forName("ir.besteveryeverapp.telegram.exoplayer.extractor.mp3.Mp3Extractor")
              .asSubclass(Extractor.class));
    } catch (ClassNotFoundException e) {
      // Extractor not found.
    }
    try {
      DEFAULT_EXTRACTOR_CLASSES.add(
          Class.forName("ir.besteveryeverapp.telegram.exoplayer.extractor.ts.AdtsExtractor")
              .asSubclass(Extractor.class));
    } catch (ClassNotFoundException e) {
      // Extractor not found.
    }
    try {
      DEFAULT_EXTRACTOR_CLASSES.add(
          Class.forName("ir.besteveryeverapp.telegram.exoplayer.extractor.ts.TsExtractor")
              .asSubclass(Extractor.class));
    } catch (ClassNotFoundException e) {
      // Extractor not found.
    }
    try {
      DEFAULT_EXTRACTOR_CLASSES.add(
          Class.forName("ir.besteveryeverapp.telegram.exoplayer.extractor.flv.FlvExtractor")
              .asSubclass(Extractor.class));
    } catch (ClassNotFoundException e) {
      // Extractor not found.
    }
    try {
      DEFAULT_EXTRACTOR_CLASSES.add(
          Class.forName("ir.besteveryeverapp.telegram.exoplayer.extractor.ogg.OggExtractor")
              .asSubclass(Extractor.class));
    } catch (ClassNotFoundException e) {
      // Extractor not found.
    }
    try {
      DEFAULT_EXTRACTOR_CLASSES.add(
          Class.forName("ir.besteveryeverapp.telegram.exoplayer.extractor.ts.PsExtractor")
              .asSubclass(Extractor.class));
    } catch (ClassNotFoundException e) {
      // Extractor not found.
    }
    try {
      DEFAULT_EXTRACTOR_CLASSES.add(
          Class.forName("ir.besteveryeverapp.telegram.exoplayer.extractor.wav.WavExtractor")
              .asSubclass(Extractor.class));
    } catch (ClassNotFoundException e) {
      // Extractor not found.
    }
    try {
      DEFAULT_EXTRACTOR_CLASSES.add(
          Class.forName("ir.besteveryeverapp.telegram.exoplayer.ext.flac.FlacExtractor")
              .asSubclass(Extractor.class));
    } catch (ClassNotFoundException e) {
      // Extractor not found.
    }

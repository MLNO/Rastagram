...
import ir.besteveryeverapp.ui.Components.CheckBoxSquare;
...
public class UserCell extends FrameLayout {
    ...
    public UserCell(Context context, int padding, int checkbox, boolean admin) {
        ...
        addView(mutualView, LayoutHelper.createFrame(22, 22, (LocaleController.isRTL ? Gravity.LEFT : Gravity.RIGHT) | Gravity.CENTER_VERTICAL,
                ...
                , 0));
        mutualView.setImageResource(ir.besteveryeverapp.telegram.R.drawable.party);
    ...
    public void setIsAdmin(int value) {
        ...
        if (value == 1) {
            adminImage.setImageResource(ir.besteveryeverapp.telegram.R.drawable.admin_star);
        } else if (value == 2) {
            adminImage.setImageResource(ir.besteveryeverapp.telegram.R.drawable.admin_star2);
        }
    }
        ...
        if (currrntStatus != null) {
            ...
            if (currentUser.bot) {
                ...
                if (currentUser.bot_chat_history || adminImage != null && adminImage.getVisibility() == VISIBLE) { //TODO fix
                    statusTextView.setText(LocaleController.getString("BotStatusRead", ir.besteveryeverapp.telegram.R.string.BotStatusRead));
                } else {
                    statusTextView.setText(LocaleController.getString("BotStatusCantRead", ir.besteveryeverapp.telegram.R.string.BotStatusCantRead));
                }
            } else {
                if (currentUser.id == UserConfig.getClientUserId() || currentUser.status != null && currentUser.status.expires > ConnectionsManager.getInstance().getCurrentTime() || MessagesController.getInstance().onlinePrivacy.containsKey(currentUser.id)) {
                    ...
                    statusTextView.setText(LocaleController.getString("Online", ir.besteveryeverapp.telegram.R.string.Online));
                } else {
                    statusTextView.setTextColor(statusColor);

...
import ir.besteveryeverapp.ui.Components.LinkPath;
import ir.besteveryeverapp.ui.Components.RadialProgress;
import ir.besteveryeverapp.ui.Components.SeekBar;
import ir.besteveryeverapp.ui.Components.SeekBarWaveform;
import ir.besteveryeverapp.ui.Components.StaticLayoutEx;
import ir.besteveryeverapp.ui.Components.TypefaceSpan;
...
public class ChatMessageCell extends BaseCell implements SeekBar.SeekBarDelegate, ImageReceiver.ImageReceiverDelegate, MediaController.FileDownloadProgressListener {
        ...
        if (messageChanged || dataChanged || isPhotoDataChanged(messageObject)) {
            ...
            if (messageObject.type == 0) {
                ...
                if (phone != null && phone.length() != 0) {
                    ...
                    phone = LocaleController.getString("NumberUnknown", ir.besteveryeverapp.telegram.R.string.NumberUnknown);
                }
    ...
    private void measureTime(MessageObject messageObject) {
        ...
        if (messageObject.messageOwner.via_bot_id == 0 && messageObject.messageOwner.via_bot_name == null && (author == null || !author.bot) && (messageObject.messageOwner.flags & TLRPC.MESSAGE_FLAG_EDITED) != 0) {
            timeString = LocaleController.getString("EditedMessage", ir.besteveryeverapp.telegram.R.string.EditedMessage) + " " + LocaleController.getInstance().formatterDay.format((long) (messageObject.messageOwner.date) * 1000);
        } else {
    ...
    private void setMessageObjectInternal(MessageObject messageObject) {
        ...
        if (drawForwardedName && messageObject.isForwarded()) {
            ...
            if (currentForwardUser != null || currentForwardChannel != null) {
                ...
                int fromWidth = (int) Math.ceil(forwardNamePaint.measureText(LocaleController.getString("From", ir.besteveryeverapp.telegram.R.string.From) + " "));
                ...
                if (viaString != null) {
                    viaNameWidth = (int) Math.ceil(forwardNamePaint.measureText(LocaleController.getString("From", ir.besteveryeverapp.telegram.R.string.From) + " " + name));
                    lastLine = AndroidUtilities.replaceTags(String.format("%s <b>%s</b> via <b>%s</b>", LocaleController.getString("From", ir.besteveryeverapp.telegram.R.string.From), name, viaUsername));
                } else {
                    lastLine = AndroidUtilities.replaceTags(String.format("%s <b>%s</b>", LocaleController.getString("From", ir.besteveryeverapp.telegram.R.string.From), name));
                }
                ...
                try {
                    ...
                    lastLine = TextUtils.ellipsize(AndroidUtilities.replaceTags(LocaleController.getString("ForwardedMessage", ir.besteveryeverapp.telegram.R.string.ForwardedMessage)), forwardNamePaint, forwardedNameWidth, TextUtils.TruncateAt.END);
        ...
        if (messageObject.isReply()) {
            ...
            if (stringFinalName == null) {
                stringFinalName = LocaleController.getString("Loading", ir.besteveryeverapp.telegram.R.string.Loading);
            }
            try {
